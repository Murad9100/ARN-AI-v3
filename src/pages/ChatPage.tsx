...
export default function ChatPage() {
  ...
  const handleSend = async () => {
    ...
    useChatStore.setState((state: any) => ({
      chats: state.chats.map((c: any) => c.id !== chatId ? c : { ...c, messages: [...(c.messages || []), assistantMessage] })
    }))
    ...
    try {
      await sendMessage(history, chunk => {
        finalContent += chunk
        const { chats } = useChatStore.getState() as any
        useChatStore.setState({
          chats: chats.map((c: any) => c.id !== chatId ? c : {
            ...c,
